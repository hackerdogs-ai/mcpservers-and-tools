"""
SonarQube SAST Scanner MCP Tool Wrapper
"""
...
try:
    from sonarqube.sonarqube_scan import SonarQubeScanner
except ImportError as e:
    SonarQubeScanner = None
    import sys
    sys.stderr.write(f"Warning: Could not import SonarQubeScanner: {e}\n")
...
def sonarqube_scan_repository(
    ...
    """
    Scan a GitHub repository with SonarQube for code quality and security analysis.
    ...
    """
    if SonarQubeScanner is None:
        return {
            "success": False,
            "error": "SonarQubeScanner not available - check application_security tools path",
            "tool": "sonarqube"
        }
    
    try:
        logger.info(f"[sonarqube_scan_repository] Starting scan for: {repo_url}")
        
        # Initialize scanner
        scanner = SonarQubeScanner(github_token=github_token)
        ...
        logger.info(f"[sonarqube_scan_repository] Scan completed: success={result.get('success', False)}")
        return result
        
    except Exception as e:
        logger.error(f"[sonarqube_scan_repository] Error: {e}", exc_info=True)
        return {
            "success": False,
            "error": str(e),
            "tool": "sonarqube"
