...
try:
    ...
    import sys
    sys.stderr.write(f"Warning: Could not import GitleaksScanner: {e}\n")
...
def gitleaks_scan_repository(
    ...
    if GitleaksScanner is None:
        return {"success": False, "error": "GitleaksScanner not available", "tool": "gitleaks"}
    try:
        logger.info(f"[gitleaks_scan_repository] Starting scan: {repo_url}")
        scanner = GitleaksScanner(github_token=github_token)
        result = scanner.scan_repository(repo_url=repo_url, scan_git_history=scan_git_history, output_format=output_format, save_output=save_output, output_dir=output_dir, timeout=timeout, github_token=github_token)
        ...
        return {"success": False, "error": str(e), "tool": "gitleaks"}
