...
logger = setup_logger(__name__, log_file_path="logs/appsec_iac_mcp.log")


def main():
    """Run IaC Security MCP server."""
    server_dir = Path(__file__).parent
    tools_dir = server_dir / "tools"
    logger.info("Starting IaC Security MCP Server...")
    server = AppSecBaseServer("appsec-iac-mcp", tools_dir)
    server.run()
